...
void Main()
{
	...
	{
		// DumpState(input, cursor);
		
		var match = matchExpr.Match(input, cursor);

		if (!match.Success)
		{
			Console.WriteLine($"No match from index {cursor}");
			ret.Append(input.Substring(cursor, input.Length - cursor));
			break;
		}
		...
		if (match.Index + match.Length + length > input.Length)
		{
			throw new Exception($"Not enough data remaining to satisfy ({repeat}x{length})");
		}

		if (match.Index > cursor)
		{
			var beforeMatch = input.Substring(cursor, match.Index - cursor);
			Console.WriteLine($"beforeMatch: {beforeMatch}");
			ret.Append(beforeMatch);
		}
		
		var toRepeat = input.Substring(match.Index + match.Length, length);

		Console.WriteLine($"toRepeat: {toRepeat}");
		
		var newText = string.Join("", Enumerable.Repeat(toRepeat, repeat));

		Console.WriteLine($"Repeated: {newText}");
		
		ret.Append(newText);
		
		cursor = match.Index + match.Length + length;
	}
