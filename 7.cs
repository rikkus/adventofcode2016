...
"
	...
	(
		line =>
		{
			// mwipinfvshxjcxe[bquehtxanardycy]om@@@@alllwhnabt[jcntxodpbdifmdxwvd]ntuocedcycagludkzzm
			bool isMatch = false;			
			
			char m = '-';
			
			bool haveAbbaInBrackets = Regex.IsMatch(line, @"\[[^\]]*([a-z])([a-z])\2\1.*\]");
			
			if (haveAbbaInBrackets)
				m = '[';
			...
			isMatch = !haveAbbaInBrackets && match.Success && (match.Groups["a"].Value != match.Groups["b"].Value);			
			
			if (isMatch)
				m = 'M';
			
			line = Regex.Replace(line, @"(?<a>.)(?<b>.)\2\1", @"@@@@");
